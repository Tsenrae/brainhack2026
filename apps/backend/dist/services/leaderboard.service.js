...
export const leaderboardService = {
    async getIndividual(userId) {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .limit(50);
        if (error)
            throw new Error(`leaderboard.getIndividual: ${error.message}`);
    ...
    async getSquads(userId) {
        ...
        const { data: squads, error } = await supabaseAdmin
            ...
            .limit(30);
        if (error)
            throw new Error(`leaderboard.getSquads: ${error.message}`);
        ...
        const entries = (squads ?? []).map((s, idx) => ({
            rank: idx + 1,
            squad_id: s.id,
            name: s.name,
            emblem: s.emblem,
