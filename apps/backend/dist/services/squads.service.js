...
export const squadsService = {
    async getAll() {
        ...
        const { data: squads, error } = await supabaseAdmin
            ...
            .limit(50);
        if (error)
            throw new Error(`squads.getAll: ${error.message}`);
        // Batch member counts
        ...
        for (const r of memberRows ?? []) {
            if (r.squad_id)
                counts.set(r.squad_id, (counts.get(r.squad_id) ?? 0) + 1);
        }
        return (squads ?? []).map(s => ({
            id: s.id,
            name: s.name,
            ...
            captain_username: s.profiles?.username ?? null,
            member_count: counts.get(s.id) ?? 0,
            total_xp: s.total_xp,
            created_at: s.created_at,
        }));
    },
    async getMySquad(userId) {
        if (isMockMode) {
        ...
        if (!profile?.squad_id)
            return null;
        const squadId = profile.squad_id;
        // Fetch squad info
        ...
        return {
            squad: {
                ...
                captain_username: squad.profiles?.username ?? null,
    ...
    async create(userId, payload) {
        ...
        const { data: profile } = await supabaseAdmin
            ...
            .single();
        if (profile?.squad_id)
            throw new Error('You are already in a squad. Leave first.');
        // Create squad
    ...
    async join(userId, squadId) {
        ...
        if (profile.squad_id === squadId)
            throw new Error('You are already in this squad.');
        if (profile.squad_id)
            await squadsService.leave(userId, profile.squad_id);
