...
const MOCK_SCAN_HISTORY = MOCK_RECENT_SCANS.map((scan, index) => ({
...
function buildMissionRecommendation(analysis) {
...
function buildReplyText(analysis, relatedMission) {
...
function recentColor(riskScore) {
    if (riskScore > 70)
        return 'red';
    if (riskScore > 40)
        return 'yellow';
...
async function fetchLinkedProfileByTelegramUserId(telegramUserId) {
    if (isMockMode) {
...
async function fetchProfileById(profileId) {
    ...
    if (error?.code === 'PGRST116')
        return null;
    if (error)
        throw new Error(`fetchProfileById: ${error.message}`);
    return data;
}
async function persistTelegramScan(profile, telegramUserId, analysis, request) {
    ...
    if (!isMockMode) {
        ...
        if (error)
            throw new Error(`persistTelegramScan: ${error.message}`);
    }
    return {
        scan_id: scanId,
        telegram_user_id: telegramUserId,
...
export const telegramService = {
    ...
    async createLinkCode(userId) {
        ...
        if (!isMockMode) {
            const { error } = await supabaseAdmin
                ...
                .eq('id', userId);
            if (error)
                throw new Error(`createLinkCode: ${error.message}`);
        }
    ...
    async analyzeMessage(request) {
        const rawAnalysis = await analyseScanPreview('text', request.content);
        const analysis = formatAnalysis(rawAnalysis);
    ...
    async getStats(userId, telegramUserId) {
        if (isMockMode) {
            return {
                messages_scanned: 23,
                threats_blocked: 8,
    ...
    async getRecentScans(userId, telegramUserId) {
        if (isMockMode)
        ...
        const { data, error } = await supabaseAdmin
            ...
            .limit(3);
        if (error)
            throw new Error(`getRecentScans: ${error.message}`);
        return (data ?? []).map(row => ({
            scan_id: row.id,
    ...
    async getScanHistory(userId, telegramUserId) {
        if (isMockMode)
        ...
        const { data, error } = await supabaseAdmin
            ...
            .limit(10);
        if (error)
            throw new Error(`getScanHistory: ${error.message}`);
        return (data ?? []).map(row => ({
            scan_id: row.id,
