...
export const usersService = {
    async createProfile(userId, email, payload) {
        ...
        return { ...data, ...computeLevel(data.xp) };
    },
    ...
    async awardXp(userId, payload) {
        ...
        const { data: current, error: fetchErr } = await supabaseAdmin
            ...
            .single();
        if (fetchErr)
            throw new Error(`awardXp fetch: ${fetchErr.message}`);
        ...
        const { data, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new Error(`awardXp update: ${error.message}`);
