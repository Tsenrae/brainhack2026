...
export const telegramController = {
  ...
  getCommands(_req: Request, res: Response): void {
  ...
  async createLinkCode(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const auth = await extractAuth(req, res);
      if (!auth) return;
  ...
  async getRecentScans(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const telegramUserId = typeof req.query.telegram_user_id === 'string' ? req.query.telegram_user_id : undefined;
      const auth = telegramUserId ? null : await extractAuth(req, res);
      if (!auth && !telegramUserId) return;
