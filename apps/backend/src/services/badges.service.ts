...
export const badgesService = {
  async getAllWithStatus(userId: string): Promise<BadgeWithStatus[]> {
    if (isMockMode) {
    ...
    if (error) throw new Error(`badges.getAllWithStatus: ${error.message}`);

    const earned = new Map(
      (data ?? []).map((r: { badge_slug: string; earned_at: string }) => [r.badge_slug, r.earned_at]),
    );
  ...
  async awardBadge(userId: string, badgeSlug: string): Promise<boolean> {
    if (!BADGE_MAP.has(badgeSlug)) return false;
    if (isMockMode) return !MOCK_EARNED.has(badgeSlug);
    ...
    if (error) throw new Error(`badges.awardBadge: ${error.message}`);
    return true;
  },

  // Award multiple badges and return slugs that were newly earned
  async awardBadges(userId: string, slugs: string[]): Promise<string[]> {
