...
// ── Mock state ─────────────────────────────────────────────────────────────────
const mockProgress = new Map<string, ScenarioProgress>();

function rankValue(rank: ScenarioRank): number {
  return { S: 4, A: 3, B: 2, C: 1 }[rank];
}

function betterRank(a: ScenarioRank | null, b: ScenarioRank): ScenarioRank {
  if (!a) return b;
...
export const scenariosService = {
  async complete(
    userId: string,
    scenarioId: string,
    payload: CompleteScenarioPayload,
    ...
    await supabaseAdmin!.from('scenario_progress').upsert(
      {
        ...
        scenario_id: scenarioId,
        status: 'completed',
        best_rank: betterRank((existing?.best_rank as ScenarioRank | null) ?? null, rank),
        best_xp: Math.max(existing?.best_xp ?? 0, xp_earned),
        best_accuracy_pct: Math.max(existing?.best_accuracy_pct ?? 0, accuracy_pct),
        attempts: (existing?.attempts ?? 0) + 1,
        last_played_at: now,
  ...
  async getProgress(userId: string): Promise<ScenarioProgress[]> {
    if (isMockMode) return Array.from(mockProgress.values());
    ...
    if (error) throw new Error(`scenarios.getProgress: ${error.message}`);

    return (data ?? []).map(r => ({
      scenario_id: r.scenario_id,
      status: r.status as ScenarioProgress['status'],
      best_rank: (r.best_rank as ScenarioRank) ?? null,
      best_xp: r.best_xp,
