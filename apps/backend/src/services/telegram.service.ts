...
const MOCK_SCAN_HISTORY: TelegramScanHistoryItem[] = MOCK_RECENT_SCANS.map((scan, index) => ({
...
const BOT_COMMANDS: TelegramBotCommand[] = [
  { command: '/start', description: 'Open the bot and get a quick welcome message' },
  { command: '/link <code>', description: 'Link your Telegram account to your ShieldVerse profile' },
  { command: '/help', description: 'See how to use the bot and what it can scan' },
...
function buildMissionRecommendation(analysis: ScanResult): TelegramMissionRecommendation {
  const lowerClassification = analysis.classification.toLowerCase();

  if (lowerClassification.includes('phishing') || lowerClassification.includes('scam')) {
    return {
  ...
  if (lowerClassification.includes('misinformation') || lowerClassification.includes('deepfake')) {
    return {
...
function buildReplyText(analysis: ScanResult, relatedMission: TelegramMissionRecommendation): string {
  const redFlags = analysis.red_flags.length > 0
    ? analysis.red_flags.slice(0, 3).map(flag => `• ${flag.title}`).join('\n')
    : '• No major red flags detected';

  const actions = analysis.recommended_actions.slice(0, 3).map(action => `• ${action.action}`).join('\n');
...
async function fetchLinkedProfileByTelegramUserId(telegramUserId: string): Promise<UserProfile | null> {
  if (isMockMode) {
    return MOCK_LINK_STATUS.telegram_user_id === telegramUserId ? (await usersService.getProfile(MOCK_LINK_STATUS.profile_id)) : null;
  }
  ...
  if (error) throw new Error(`fetchLinkedProfileByTelegramUserId: ${error.message}`);
  return data as UserProfile;
}

async function fetchProfileById(profileId: string): Promise<UserProfile | null> {
  if (isMockMode) {
    return profileId === MOCK_LINK_STATUS.profile_id ? await usersService.getProfile(profileId) : null;
  }
...
async function persistTelegramScan(profile: UserProfile, telegramUserId: string, analysis: TelegramAnalysisResponse, request: TelegramAnalyzeRequest): Promise<TelegramScanHistoryItem> {
  const scannedAt = new Date().toISOString();
  const scanId = `telegram-${crypto.randomUUID()}`;
  const preview = request.content.length > 90 ? `${request.content.slice(0, 87)}...` : request.content;
...
export const telegramService = {
  ...
  async createLinkCode(userId: string): Promise<TelegramLinkCodeResponse> {
    ...
    const expiresAt = new Date(Date.now() + LINK_CODE_TTL_HOURS * 60 * 60 * 1000).toISOString();
  ...
  async linkTelegramAccount(payload: TelegramLinkRequest): Promise<TelegramLinkStatus> {
    ...
    if (lookupError) throw new Error(`linkTelegramAccount lookup: ${lookupError.message}`);
    ...
    if (conflictError) throw new Error(`linkTelegramAccount conflict: ${conflictError.message}`);
    if (existing && existing.id !== profile.id) {
      throw new Error('This Telegram account is already linked to another profile');
    }
    ...
    if (updateError) throw new Error(`linkTelegramAccount update: ${updateError.message}`);
  ...
  async analyzeMessage(request: TelegramAnalyzeRequest): Promise<TelegramAnalysisResponse> {
    ...
    const persistedScan = await persistTelegramScan(profile, telegramUserId, analysis, request);
    const updatedProfile = await usersService.awardXp(profile.id, { amount: TELEGRAM_XP_PER_SCAN, reason: 'Telegram bot scan' });
  ...
  async getStats(userId?: string, telegramUserId?: string): Promise<TelegramStats> {
    if (isMockMode) {
      return {
        messages_scanned: 23,
        threats_blocked: 8,
    ...
    return {
      ...
      xp_earned: rows.reduce((sum, row) => sum + Number(row.xp_awarded ?? 0), 0),
    };
  },

  async getRecentScans(userId?: string, telegramUserId?: string): Promise<TelegramRecentScan[]> {
    ...
    const { data, error } = await supabaseAdmin!
      .from('telegram_scan_history')
      .select('id, content_preview, risk_score, classification, threat_level, scanned_at')
      .eq('profile_id', profile.id)
  ...
  async getScanHistory(userId?: string, telegramUserId?: string): Promise<TelegramScanHistoryItem[]> {
    ...
    const { data, error } = await supabaseAdmin!
      .from('telegram_scan_history')
      .select('id, telegram_user_id, telegram_username, content_preview, risk_score, classification, threat_level, xp_awarded, scanned_at')
      .eq('profile_id', profile.id)
      .order('scanned_at', { ascending: false })
      .limit(10);
